...
const runQsmPipeline = async (request: Request, response: Response) => {
  ...
  const subjectsFromCohort: Cohort[] = await Promise.all(
    (cohorts || []).map(database.cohorts.get.byName),
  );
  ...
  const linkedQsmJob = await jobHandler.addJobToQueue(
    JobType.QSM,
    qsmParameters,
    null,
    description,
