...
const updateCohort = async (request: Request, response: Response) => {
  ...
  const removedSubjects = cohort.subjects.filter(
    (savedSubject) =>
      !(subjects as string[]).find((subject) => subject == savedSubject),
  );
  const newSubjects = (subjects as string[]).filter(
    (subject) =>
      !cohort.subjects.find((savedSubject) => savedSubject === subject),
  );
