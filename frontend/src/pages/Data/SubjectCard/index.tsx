...
const subjectHelperText = (
  <Text>
    Data for running the QSMxT pipeline on is stored in the form of subjects.
    <br />
    A subject refers to a patient or research subject from which the QSM data
    <br />
    was sourced.
  </Text>
);
...
const explanatoryText = (
  <Text>
    <i>
      Select a Subject on the left of the screen to view images and details of
...
const SubjectCard: React.FC = () => {
  ...
  const renderSubjectDetail = (
    ...
    const SessionImages = subject.dataTree.sessions[
      firstSessionName
    ].sessionImages.filter((img) => img);
    const firstRunName = Object.keys(
      subject.dataTree.sessions[firstSessionName].runs,
    )[0];
    const numberOfRuns = Object.keys(
      subject.dataTree.sessions[firstSessionName].runs,
    ).length;
    const numberOfEchoes = Object.keys(
      subject.dataTree.sessions[firstSessionName].runs[firstRunName]?.echoes ??
        {},
    ).length;
    return (
      <div style={{ minWidth: "100%", maxWidth: "100%" }}>
        ...
        >
          ...
          <div>
            <Button
              type="link"
              onClick={navigateToSubjectResults(selectedSubject)}
            >
              View Results
            </Button>
            <Popconfirm
              ...
              description={
                <div>This will also delete all assoicated QSM results</div>
              }
        ...
        {openRunDetailButtonDisabled ? (
          <Popover
            title={null}
            content={
              "Select a session for the subject on the left of the screen in order to be able to open it"
            }
          >
            {openRunButton}
          </Popover>
  ...
  const renderNoSelectedSubjectText = (subjects: Subject[]) => {
    return (
      <div>
        <Paragraph>
          Currently you have data uploaded into <b>QSMxT</b> for{" "}
