...
const QsmRunStarter: React.FC<{}> = () => {
  const { subjects, cohorts, selectedSubjects, selectedCohorts } =
    useContext(context);
  const [createSegmentation, setCreateSegmentation]: [any, any] =
    useState(false);
  ...
  selectedCohorts.forEach((cohort) => {
    subjectsFromCohorts.push(
      ...((cohorts || {})[cohort] || { subjects: [] }).subjects,
    );
  });
  ...
  const runQsm = async () => {
    await apiClient.runQsmPipeline(
      sessions,
      runs,
      pipelineConfig,
      selectedSubjects,
  ...
  return (
    ...
    >
      ...
      {!subjectsFromCohorts.length && !selectedSubjects.length ? (
        <div>
          <i>
            Select Subjects and Cohorts on the left of the screen to add to the
            QSM run
          </i>
          <br />
        </div>
      ...
      >
        {Array.from(allSessions).map((sessionName) => (
          <Option key={sessionName}>{sessionName}</Option>
        ))}
      </Select>
      <br />
      <br />
      ...
      <Select
        ...
        style={{ width: "100%" }}
        placeholder={
          useAllRuns ? "Using all runs" : "Please select the runs to include..."
        }
        value={runs}
        onChange={(runs) => {
          setRuns(runs);
      ...
      >
        {Array.from(allRuns).map((runName) => (
          <Option key={runName}>{runName}</Option>
        ))}
      </Select>
      <br />
      <br />
