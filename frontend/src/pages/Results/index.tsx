...
const Results: React.FC<{}> = () => {
  const { selectedSubjects, selectedCohorts, navigate, cohorts, qsmResults } =
    React.useContext(context);

  const allSelectedSubjects = selectedSubjects.map(
  ...
  const linkedQsmJobs = (qsmResults || []).filter((qsmResult: any) => {
    return qsmResult.parameters.subjects.find((subject: string) => {
      const linkedByCohort = selectedCohorts.filter((cohortName) =>
        (cohorts || {})[cohortName].subjects.find((x) => x === subject),
      ).length;
      const linkedBySubject = selectedSubjects
        .map((x) => x.split("&")[0])
        .find((x) => x === subject);
    ...
    : [
        {
          label: !qsmResults
            ...
            : "No results for selected subjects",
          key: "empty",
  ...
  return (
    <PageContainer title={"Results"} gap={0}>
      <Tabs style={{ minWidth: "100%" }} type="card" items={tabItems} />
    </PageContainer>
  );
};
