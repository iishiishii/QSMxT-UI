  ...
  const renderBody = (qsmResult: QsmResult | null) => {
    if (!selectedSubjects.length && !selectedCohorts.length) {
      ...
      const {
        ...
        startedAt,
        qsmFinishedAt,
        segmentationFinishedAt,
        segmentationCreatedAt,
        parameters,
      } = qsmResult;
      return (
        <Descriptions title={null} bordered>
          <Descriptions.Item label="Run Description">
            {description}
          </Descriptions.Item>
          <Descriptions.Item label="Started At">
            {startedAt
              ? moment(startedAt).format("MMMM Do YYYY, h:mm:ss a")
              : "Not started yet"}
          </Descriptions.Item>
          <Descriptions.Item label="QSM Finished At">
            {qsmFinishedAt
              ? moment(qsmFinishedAt).format("MMMM Do YYYY, h:mm:ss a")
              : "Not finished"}
          </Descriptions.Item>
          {segmentationCreatedAt && (
            <Descriptions.Item label="Segmentation Finished At">
              {segmentationFinishedAt
                ? moment(segmentationFinishedAt).format(
                    "MMMM Do YYYY, h:mm:ss a",
                  )
                : "Not finished"}
            </Descriptions.Item>
          )}
          <Descriptions.Item label="Sessions Included">
            {parameters.sessions.length || "All"}
          </Descriptions.Item>
          <Descriptions.Item label="Runs Included">
            {parameters.runs.length || "All"}
          </Descriptions.Item>
          <Descriptions.Item label="QSM Config">
