...
const QsmImages: React.FC<Props> = ({
  qsmResult,
  allSelectedSubjects,
  loading,
}) => {
  const { selectedSubjects, selectedCohorts, navigate, cohorts } =
  ...
  const renderQsmResults = (qsmResult: QsmResult) => {
    return (
      <Collapse defaultActiveKey={[]}>
        {qsmResult.qsmImages
          .filter((image: string) =>
            allSelectedSubjects.find((subject: string) =>
              image.includes(subject),
            ),
          )
          .map((image: string) => {
            const subject = image
            ...
            return (
              <Panel
                ...
                key={image}
              >
                <NiiVue imageUrl={image} type="qsm" />
              </Panel>
            );
          })}
      </Collapse>
  ...
  const renderBody = (qsmResult: QsmResult | null) => {
    if (!selectedSubjects.length && !selectedCohorts.length) {
      return (
        <i>
          Select subjects or cohorts on the left of the scren to view their QSM
          images{" "}
        </i>
      );
    } else if (!qsmResult) {
