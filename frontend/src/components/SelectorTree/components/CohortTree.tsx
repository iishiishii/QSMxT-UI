...
const CohortTree: React.FC<{}> = () => {
  const { cohorts, selectedCohorts, setSelectedCohorts, navigate, page } =
    useContext(context);
        ...
        if (!selectedCohorts.find((cohort) => cohort === clickedCohort)) {
          ...
          setSelectedCohorts(
            selectedCohorts.filter(
              (selectedCohort) => selectedCohort !== selectedKey,
            ),
          );
          setExpandedKeys(
            selectedCohorts.filter(
  ...
  const onSelect: DirectoryTreeProps["onSelect"] = selectTreeNode(
    selectedCohorts,
    setSelectedCohorts,
    setExpandedKeys,
    page,
  );
  const onExpand: DirectoryTreeProps["onSelect"] = selectTreeNode(
    selectedCohorts,
    setSelectedCohorts,
    setExpandedKeys,
    page,
  ...
  if (!Object.keys(cohorts as Cohorts).length) {
    return (
      <div style={{ minHeight: 250 }}>
        <div>
          Cohorts group together subjects for QSM reconstruction and analysis.
          <br />
          Go to <a onClick={() => navigate("/data")}>Data</a> to create a cohort
          and add subjects.
        </div>
      </div>
    );
